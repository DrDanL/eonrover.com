...
async function main() {
  try {
    ...
    const shipyardStarted = "'2026-11-30T23:59:00.000Z'::timestamp";
    ...
    await sql(`INSERT INTO "ShipyardQueueItem" ("id","planetId","itemKey","itemType","canonicalDefenceKey","quantity","remaining","costAlloy","costHeliox","costAether","durationSeconds","startedAt","completesAt","status","jobId") VALUES ('q-legacy','p-a','legacy-item','ship',NULL,2,2,400,200,0,60,${shipyardStarted},${updated},'PENDING','legacy-job')`);
    const missionValues = `("id","originId","targetId","targetGalaxy","targetSystem","targetSlot","missionType","ships","cargo","speedPercent","departedAt","arrivesAt","returnsAt","status")`;
    await sql(`INSERT INTO "FleetMission" ${missionValues} VALUES ('m-legacy','p-a','p-d',1,55,2,'ATTACK','{}','{}',100,${updated},${updated},${updated},'OUTBOUND')`);
    await sql(`INSERT INTO "FleetMission" ${missionValues} VALUES ('m-corvette','p-a','p-d',1,55,2,'ATTACK','{"corvette":2}','{}',100,${updated},${updated},${updated},'OUTBOUND')`);
    const corvette = await rows(`SELECT "id" FROM "FleetMission" WHERE "id"='m-corvette'`);
    ...
    const names = (await rows(`SELECT indexname FROM pg_indexes WHERE tablename IN ('FleetMission','FrigateStrikeReport') AND indexname ILIKE '%frigate%'`)).map((row) => row.indexname);
