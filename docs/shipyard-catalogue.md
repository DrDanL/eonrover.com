...
Stage 7B restores API-only, authoritative Ship batches. A batch is one canonical ship key and an integer quantity from 1 through 100. PostgreSQL stores the accepted total resource costs, quantity, duration, timestamps and terminal status; it enforces one pending batch per planet with a partial unique index. Start and cancellation lock account → planet → queue in serializable transactions. Cancellation refunds exactly 50% of the persisted totals. Redis receives only a post-commit deterministic wake-up named `ship-<queue-item-id>`; a scheduling/removal failure cannot change the committed database state. The player page remains read-only until Stage 7D. Stage 7C will replace legacy completion with PostgreSQL-authoritative completion and reconciliation.
