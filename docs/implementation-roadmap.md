...
**Stage 16C2c (Galaxy hand-off complete):** canonical Frigate-only, no-loot strikes have separate authenticated command, launch, active-state, and immutable attacker-owned report APIs, plus compact Fleet launch and existing-Reports-page result views. Public, unprotected same-galaxy Galaxy slots now offer a coordinate-only Frigate hand-off that pre-fills the Fleet Frigate Strike mode; the server still confirms availability, protection, fuel, timing, and combat. Planetary Shield construction, mixed Corvette/Frigate manifests, loot, debris, raids, and generic Fleet processing remain unavailable. The physical pre-Stage-16 migration replay and disposable real-process Frigate recovery smoke remain deferred verification.
