...
**Stage 16C2b (report screen complete):** canonical Frigate-only, no-loot strikes have separate authenticated command, launch, active-state, and immutable attacker-owned report APIs, plus compact Fleet launch and existing-Reports-page result views. Players enter same-galaxy coordinates and a bounded Frigate quantity, review only server-provided command eligibility/fuel/duration, track authoritative active timing, and read immutable result snapshots without live target or resolver data. Galaxy coordinate hand-off remains pending; Planetary Shield construction, mixed Corvette/Frigate manifests, loot, debris, raids, and generic Fleet processing remain unavailable. The physical pre-Stage-16 migration replay and disposable real-process Frigate recovery smoke remain deferred verification.
