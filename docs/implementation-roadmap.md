...
**Stage 16C1 (complete, API only):** canonical Frigate-only, no-loot strikes now have separate authenticated command, launch, active-state, and immutable attacker-owned report APIs. The API exposes coordinate-only target input and allowlisted mission/report projections while the PostgreSQL lifecycle, deterministic wake-ups, and restart reconciliation remain authoritative. No Frigate player UI is active yet; Planetary Shield construction, mixed Corvette/Frigate manifests, loot, debris, raids, and generic Fleet processing remain unavailable. The physical pre-Stage-16 migration replay and disposable real-process Frigate recovery smoke remain deferred verification.
