...
**Stage 20A (runtime guardrails complete):** API and worker production configuration reject known placeholder credentials without echoing secrets. Liveness remains dependency-free; bounded readiness exposes only `ok` or `unavailable`. Worker startup, queue registration, and canonical reconciliation emit structured safe operational events. The operator runbook documents deployment and wake-up recovery without claiming a backup/restore rehearsal or production deployment.
