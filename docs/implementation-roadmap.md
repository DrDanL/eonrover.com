...
**Stage 16C2a (Fleet launch screen complete):** canonical Frigate-only, no-loot strikes have separate authenticated command, launch, active-state, and immutable attacker-owned report APIs, plus a compact Fleet-page Frigate Strike mode. Players enter same-galaxy coordinates and a bounded Frigate quantity, review only server-provided command eligibility/fuel/duration, and can track authoritative active timing. Galaxy coordinate hand-off and Frigate report rendering remain pending; Planetary Shield construction, mixed Corvette/Frigate manifests, loot, debris, raids, and generic Fleet processing remain unavailable. The physical pre-Stage-16 migration replay and disposable real-process Frigate recovery smoke remain deferred verification.
