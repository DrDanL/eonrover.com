...
- Stage 13B1–B3 adds a typed defence catalogue and canonical Flak Turret marker. The existing Shipyard API may start only validated Flak batches; legacy defence rows remain inert, while cancellation, completion, deterministic wake-ups and bounded recovery reuse the established PostgreSQL-authoritative Shipyard lifecycle. Rail Battery and Planetary Shield remain catalogue-only.
- Stage 13C1–C2 extends that same Shipyard read model with an allowlisted defence projection for the player page. It exposes canonical availability, ownership, requirements, affordability, accepted presentation values and active-queue refund state without exposing queue internals; the Reports page continues to render immutable canonical defence snapshots separately from live inventory.
- Stage 15B activates Rail Battery as a second canonical Shipyard defence only when Shipyard level 4 and completed account-wide Weapon Technology level 2 are present. It shares Flak's one-pending-batch, persisted-cost/refund, deterministic wake-up, completion and recovery lifecycle.
...
- Stage 20A–20B1 keeps existing local/test defaults but rejects known placeholder production credentials at the API and worker configuration boundaries. API and worker liveness is dependency-free; readiness makes bounded PostgreSQL/Redis checks but returns only `ok` or `unavailable`. `apps/worker/src/operationalEvents.ts` projects startup, queue registration, job outcome, and reconciliation events to static operation families, safe statuses, and bounded counts without queue names, identifiers, payloads, raw errors, or connection details. A disposable physical PostgreSQL backup/restore and Frigate Redis-loss recovery rehearsal has passed; it does not claim production recovery.
...
| Combat | Canonical lifecycle, allowlisted API, and bounded player controls complete; legacy prototype remains unsafe | Fixed-speed Corvette-only launch reserves ships and round-trip Heliox against active, verified, unprotected same-galaxy targets. New launches persist the v2 seeded resolver policy; accepted v1 missions retain their historical six-round policy. V2 runs until elimination, a proven no-damage stalemate, or its 512-round typed unresolved safety cap. A serializable arrival transaction locks target forces, applies losses, writes an immutable canonical result, and creates paired safe notifications; only surviving Corvettes return. Deterministic dedicated queue wake-ups, a processor, and startup plus 30-second bounded reconciliation recover missed arrivals/returns while PostgreSQL remains authoritative. Galaxy offers a coordinate-only Strike hand-off; Fleet submits only origin, coordinates, and Corvette count; Reports renders only immutable canonical combat results without seeds or resolver versions. Loot, cargo, debris, raids, recalls, and generic Fleet combat remain unavailable. |
...
| Administration and moderation | Read-only diagnostics only | The retained `/admin` shell plus `/api/admin/status`, `/api/admin/overview`, and bounded `/api/admin/players?query=…` are ADMIN-only and local/test by default; production requires `ADMIN_PORTAL_ENABLED=true`. Player-state detail, account/configuration/announcement/queue/audit/security/health management routes remain retired with a safe unavailable response. |
...
| `/game/galaxy` → `/game/planets/:planetId/fleet?mode=frigate-strike` | Player | Public unprotected same-galaxy slots offer only a coordinate-only Prepare Frigate Strike handoff. Fleet validates bounded URL coordinates before pre-filling Frigate Strike mode, then uses the authoritative command API for owned inventory, eligibility, fuel, timing, and launch. |
...
| `GET /api/admin/status` | Current ADMIN, local/test or explicit opt-in | Returns exactly `{ "status": "read-only" }` and performs no database, Redis, queue, or player-state work. |
| `GET /api/admin/overview` | Current ADMIN, local/test or explicit opt-in | Read-only aggregate counts for account status/verification, owned planets, and valid active canonical outbound, returning, and transport capacity-wait operations. It exposes no identifiers, mission rows, queue state, or target/player detail. |
| `GET /api/admin/players?query=…` | Current ADMIN, local/test or explicit opt-in | Strict, bounded (25) username-or-email lookup with only username, email, account status, and verification boolean; there is no player-detail endpoint. |
...
| `ShipyardQueueItem` | Planet, item key/type, quantity/remaining, accepted cost/timing/status/job. Legacy `itemType` remains permissive, but new trusted Flak batches carry nullable legacy-safe `canonicalDefenceKey = flakTurret`; only that marker permits a defence inventory increment. |
...
The README's backup commands are likely broken when values exist only in `.env`: host-shell expansion of `$POSTGRES_USER`/`$POSTGRES_DB` occurs before Compose applies `.env` to the service. Stage 20B nevertheless passed a separate disposable physical PostgreSQL backup/restore and canonical Frigate Redis-loss recovery rehearsal using container-native tooling; the README examples themselves remain unvalidated and not automated.
...
Canonical Frigate strikes are isolated from historical Corvette and legacy Fleet rows: shared Frigate planning/resolution and the dedicated arrival scheduler live under `packages/shared/src`; internal launch/scheduling wrappers live under `apps/api/src/services`; and the worker consumes only `frigate-strike-arrival-queue` with a dedicated 30-second bounded reconciler. New launches persist `frigate-strike-v2`, which combines all surviving Frigate fire into one seeded salvo only while a Planetary Shield remains; accepted v1 rows retain their original policy. `apps/api/src/routes/fleet.ts` exposes separate allowlisted Frigate command, active-state, and coordinate-only launch routes, while `apps/api/src/routes/frigateStrikeReports.ts` exposes only immutable attacker-owned Frigate reports. The existing Fleet page contains a compact Frigate Strike mode, and the existing Reports page renders immutable result list/details including Shield forces. Public same-galaxy Galaxy slots hand off only bounded coordinates; `combatTargetEligibility.ts` remains the server authority and Fleet alone presents safe target-rejection reasons. Planetary Shield uses the existing Shipyard route/page and queue lifecycle. Physical pre-Stage-16 migration replay remains deferred. Stage 21A2b passed exact-390px active Frigate Fleet and simultaneous Mission Control operation checks; broader end-to-end mobile verification remains deferred.

`apps/api/src/routes/operations.ts` provides the separate read-only `GET /api/operations` Mission Control contract. It merges only valid active canonical mission snapshots across Deploy, Colonisation, Transport, Probe, Corvette Strike, and Frigate Strike; it emits no mission identifiers, queue data, target identity, combat state, legacy JSON, or actions. `/game/operations` renders the same safe status data with outbound/return grouping and presentation-only countdowns. Stage 21A2b verified simultaneous outbound Frigate and returning Transport presentation at an exact 390px viewport with no horizontal overflow.
...
4. Audit retention is not durable. A disposable backup/restore and canonical Frigate wake-up recovery rehearsal has passed, and the runbook now documents readiness and safe operational logging; production CI/CD, TLS, monitoring, rollback automation, and production incident rehearsal remain absent.
