...
Canonical Frigate strikes are isolated from historical Corvette and legacy Fleet rows: shared Frigate planning/resolution and the dedicated arrival scheduler live under `packages/shared/src`; internal launch/scheduling wrappers live under `apps/api/src/services`; and the worker consumes only `frigate-strike-arrival-queue` with a dedicated 30-second bounded reconciler. `apps/api/src/routes/fleet.ts` exposes separate allowlisted Frigate command, active-state, and coordinate-only launch routes, while `apps/api/src/routes/frigateStrikeReports.ts` exposes only immutable attacker-owned Frigate reports. No player-facing Frigate screen is introduced by this API stage. Physical pre-Stage-16 migration replay and disposable real-process recovery smoke verification remain deferred.
