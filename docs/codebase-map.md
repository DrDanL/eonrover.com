...
| Combat | Canonical launch foundation prepared; legacy prototype remains unsafe | A fixed-speed Corvette-only shared planner and deterministic `corvette-strike-v1` resolver now provide pure inputs for a future adversarial lifecycle. Nullable canonical strike snapshots, a one-active-per-origin database invariant, and an internal serializable launch service reserve Corvettes and round-trip Heliox only for active, verified, unprotected same-galaxy targets. Arrival resolution, reports, queues, workers, recovery, player APIs/UI, loot, debris, raids, and recalls remain unavailable; legacy generic Fleet combat stays contained. |
