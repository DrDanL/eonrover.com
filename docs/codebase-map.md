...
| Colonisation | Prepared, unavailable | Colony Ships have an attainable Shipyard-only prerequisite. Internal serializable launch reserves one ship and authoritative Heliox, persists canonical target/timing/characteristics/starter snapshots, and uses account plus coordinate locks and PostgreSQL constraints to protect the planet limit and target claim. Its post-commit best-effort wake-up uses a dedicated dormant colonisation queue; PostgreSQL remains authoritative if Redis fails. Internal arrival completion consumes only persisted snapshots to create one colony at its due time, or records an occupied-target terminal failure while restoring the one reserved ship (never fuel). Homeworld allocation skips active canonical reservations. The generic legacy mission remains disabled; no worker consumer, recovery, player route, or UI is active. |
