...
This document was refreshed for the Stage 5C1 building-prerequisite milestone on 2026-09-06. Areas outside the trusted slice retain the original 2026-09-04 prototype assessment unless explicitly updated below.
...
| Buildings and construction queues | Implemented for the trusted slice | One active building item per planet is enforced. Start derives ordered building prerequisites only from persisted completed levels, then checks projected energy under the planet lock before settlement/deduction; pending work and browser-supplied levels never satisfy a gate. The allowlisted API and categorised web interface expose complete requirements, current levels, actionable cross-category links, costs, effects, availability priority, countdown and refund information without internal job IDs. Field capacity and general ordered multi-item queues remain future work. |
...
| `GET /api/planets/:planetId/buildings` | Owning player | Connected; settles due completion/production and returns allowlisted energy/category metadata, authoritative balances/rates/storage, every ordered building prerequisite with its current/completed level, per-building effects/projections/eligibility, and public active-construction data without queue job IDs. Availability priority is active construction, prerequisites, energy, then resources. |
| `POST /api/planets/:planetId/buildings` | Owning player | Connected; after due completion and under a planet lock, recomputes authoritative completed levels, rejects all missing building prerequisites with `PREREQUISITES_NOT_MET`, and then blocks only demand-increasing upgrades whose projected demand exceeds supply, all before resource settlement/deduction/row creation/scheduling. It then enforces resources and atomically snapshots accepted cost. |
...
| Shared, 49 tests | Existing formulas plus deterministic timestamp production, precision, validation, central current/projected energy, hard-gate edge cases, category mapping, ordered building-only prerequisites, visual projection/storage timing, next-action selection and planet-switch routing. | Broader combat/research/shipyard/fleet formula edge cases. |
...
| API integration, 129 tests | Existing routes plus atomic registration, email recovery, session hardening, prerequisite/energy-gated resource and building concurrency/completion, overdue prerequisite settlement, structured lock details, command-summary consistency/ownership/allowlisting, and admin player-state RBAC/allowlists/audit. | Research/shipyard/fleet/social concurrency and complete authorization matrices. |
