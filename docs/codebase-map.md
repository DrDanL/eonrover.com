...
| Espionage | Planning prepared; legacy prototype remains unsafe | Shared rules now constrain a future intelligence-only mission to one Probe at fixed speed between distinct bounded same-galaxy coordinates, with deterministic outbound/return travel snapshots. Legacy reports and generic mission behavior remain unavailable for player use: any ship can run the prototype, reports disclose too much, target resources can be stale, and detection is unconditional. |
