...
| Espionage | Internal lifecycle, worker recovery, and deterministic wake-ups prepared; legacy prototype remains unsafe | Shared rules constrain a future intelligence-only mission to one Probe at fixed speed between distinct bounded same-galaxy coordinates, with deterministic outbound/return travel snapshots. Nullable canonical Probe origin/target/account references, manifest/fuel/duration snapshots, and a separate lifecycle phase persist alongside legacy rows under a one-active-per-origin database constraint. The internal launch transaction resolves only an active, verified, unprotected cross-player target by coordinates and reserves one Probe plus accepted round-trip Heliox. A due canonical arrival settles target production at its persisted boundary, creates one immutable relational report from the deterministic allowlisted disclosure policy, and sends one safe notification to each participant; only a due return restores the reserved Probe. A dedicated worker consumes only deterministic Probe wake-ups, and startup plus 30-second non-overlapping reconciliation settles overdue canonical phases and restores missing future jobs. PostgreSQL remains authoritative if Redis fails. No public API, report page, or player control exists. Legacy reports and generic mission behavior remain unavailable for player use: any ship can run the prototype, reports disclose too much, target resources can be stale, and detection is unconditional. |
