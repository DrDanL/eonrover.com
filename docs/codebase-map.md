...
- Stage 15B activates Rail Battery as a second canonical Shipyard defence only when Shipyard level 4 and completed account-wide Weapon Technology level 2 are present. It shares Flak's one-pending-batch, persisted-cost/refund, deterministic wake-up, completion and recovery lifecycle; the marker database constraint permits only null, `flakTurret`, or `railBattery`, so legacy defence rows remain inert. Planetary Shield remains coming later.
...
Canonical Frigate strikes are isolated from historical Corvette and legacy Fleet rows: shared Frigate planning/resolution and the dedicated arrival scheduler live under `packages/shared/src`; internal launch/scheduling wrappers live under `apps/api/src/services`; and the worker consumes only `frigate-strike-arrival-queue` with a dedicated 30-second bounded reconciler. `apps/api/src/routes/fleet.ts` exposes separate allowlisted Frigate command, active-state, and coordinate-only launch routes, while `apps/api/src/routes/frigateStrikeReports.ts` exposes only immutable attacker-owned Frigate reports. The existing Fleet page contains a compact Frigate Strike mode, and the existing Reports page renders its immutable Frigate result list/details; both use safe API projections only. Public, unprotected same-galaxy Galaxy slots hand off only bounded coordinates to the Frigate mode; target eligibility remains server-authoritative. Physical pre-Stage-16 migration replay and disposable real-process recovery smoke verification remain deferred.
