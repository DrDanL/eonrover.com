...
| Fleet missions and travel | Partially implemented, player screen unavailable | Legacy generic Fleet routes return an authenticated unavailable boundary and no worker consumes legacy jobs. Authenticated owned-planet DEPLOY command-summary and launch routes expose only safe authoritative state, while internal launch dispatches a dedicated deterministic wake-up after commit; its worker reloads PostgreSQL and completes canonical arrivals idempotently. Startup/30-second reconciliation restores missing wake-ups and completes overdue canonical rows; the Fleet player screen/navigation remain unavailable pending Stage 8C2. |
