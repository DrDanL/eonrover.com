...
| Colonisation | Prepared, unavailable | Colony Ships have an attainable Shipyard-only prerequisite, and shared same-system planning, deterministic characteristics, and explicit starter-state rules are ready for a future trusted lifecycle. The generic legacy mission remains disabled; no player colonisation route, completion, or recovery is active. |
