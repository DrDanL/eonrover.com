...
| `colonization-arrival-queue` | Internal Stage 9B5a launch and Stage 9B5b worker | A deterministic canonical-colonisation wake-up is dispatched after launch commits. The worker accepts only a mission ID, reloads persisted state, completes due arrivals through the shared transaction, and reschedules early jobs from persisted arrival time. No startup repair or reconciliation exists yet. |
