...
| Colonisation | Prepared, unavailable | Colony Ships have an attainable Shipyard-only prerequisite, while canonical reservation snapshots, shared same-system planning, deterministic characteristics, starter-state rules, and transaction-scoped coordinate locking are ready for a future trusted lifecycle. Homeworld allocation now skips active canonical reservations. The generic legacy mission remains disabled; no player colonisation route, launch, completion, or recovery is active. |
