...
| Espionage | Internal launch, disclosure policy, and canonical report persistence prepared; legacy prototype remains unsafe | Shared rules constrain a future intelligence-only mission to one Probe at fixed speed between distinct bounded same-galaxy coordinates, with deterministic outbound/return travel snapshots. Nullable canonical Probe origin/target/account references, manifest/fuel/duration snapshots, and a separate lifecycle phase persist alongside legacy rows under a one-active-per-origin database constraint. The internal launch transaction resolves only an active, verified, unprotected cross-player target by coordinates and reserves one Probe plus accepted round-trip Heliox. A pure disclosure policy maps the existing Espionage Technology accuracy score to deterministic tiers and builds reports only from allowlisted public identity, resources, completed buildings, ships, and defences. One immutable relational canonical report snapshot may be stored for each canonical Probe mission, with explicit mission, attacker, and target references. No public API, queue, worker, report completion, notification, or player control exists. Legacy reports and generic mission behavior remain unavailable for player use: any ship can run the prototype, reports disclose too much, target resources can be stale, and detection is unconditional. |
