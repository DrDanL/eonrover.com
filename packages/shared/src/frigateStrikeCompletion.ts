...
import { FRIGATE_STRIKE_V1_RESOLVER_VERSION, FRIGATE_STRIKE_V2_RESOLVER_VERSION, resolveFrigateStrike } from './frigateStrike';
...
function canonical(mission: any) {
  ...
  const version = mission?.frigateStrikeResolverVersion;
  if (!ships || !attackerTechnology || mission?.missionType !== 'ATTACK' || (version !== FRIGATE_STRIKE_V1_RESOLVER_VERSION && version !== FRIGATE_STRIKE_V2_RESOLVER_VERSION) || typeof mission.frigateStrikeResolverSeed !== 'string' || !mission.frigateStrikeResolverSeed || !whole(mission.frigateStrikeOutboundFuelHeliox) || !whole(mission.frigateStrikeReturnFuelHeliox) || !Number.isSafeInteger(mission.frigateStrikeOutboundDurationSeconds) || mission.frigateStrikeOutboundDurationSeconds <= 0 || !Number.isSafeInteger(mission.frigateStrikeReturnDurationSeconds) || mission.frigateStrikeReturnDurationSeconds <= 0 || mission.speedPercent !== 100 || !mission.departedAt || !mission.arrivesAt || mission.arrivesAt.getTime() - mission.departedAt.getTime() !== mission.frigateStrikeOutboundDurationSeconds * 1000) return null;
  return { ships, attackerTechnology, version };
}
  ...
  for (let attempt = 0; attempt < ATTEMPTS; attempt += 1) try {
    const outcome = await database.$transaction(async (tx) => {
      ...
      if (mission.frigateStrikePhase === 'OUTBOUND' && mission.status === 'OUTBOUND') {
        ...
        const result = resolveFrigateStrike({ version: accepted.version, seed: mission.frigateStrikeResolverSeed, attacker: { frigates: accepted.ships.frigate, technology: accepted.attackerTechnology }, defender: { ships: quantities(ships, SHIPS), defences: quantities(defences, DEFENCES), technology: defenderTechnology(research) } });
        ...
        await tx.frigateStrikeReport.create({ data: { missionId: mission.id, attackerId: origin.ownerId, defenderId: target.ownerId, createdAt: currentTime, resolverVersion: accepted.version, resultSnapshot: snapshot } });
