 ...
 * lock order.
 */
async function completeShipyardBatch(database, queueItemId, now = new Date()) {
    for (let attempt = 0; attempt < 3; attempt += 1) {
        try {
            return await database.$transaction(async (transaction) => {
                ...
                if (!item || !planet || item.planetId !== planet.id)
                    return 'missing';
...
async function completeDueShipyardForPlanet(database, planetId, now = new Date()) {
