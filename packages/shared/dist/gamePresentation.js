...
exports.projectVisualResourceAmount = projectVisualResourceAmount;
exports.timeUntilStorageFullSeconds = timeUntilStorageFullSeconds;
exports.selectPlanetNextAction = selectPlanetNextAction;
exports.planetIdFromGamePath = planetIdFromGamePath;
exports.planetSwitchPath = planetSwitchPath;
function projectVisualResourceAmount(input) {
    const values = Object.values(input);
    if (!values.every(Number.isFinite))
        throw new RangeError('Resource projection values must be finite');
...
function timeUntilStorageFullSeconds(amount, hourlyRate, capacity) {
...
function selectPlanetNextAction(input) {
    ...
    if (input.energyStatus === 'deficit' || input.energyBlockedBuildingKeys.length > 0) {
        return {
            ...
            reason: input.energyStatus === 'deficit'
...
function planetIdFromGamePath(pathname) {
    return PLANET_ROUTE.exec(pathname)?.[1] ?? null;
}
function planetSwitchPath(pathname, currentPlanetId, nextPlanetId) {
    const match = PLANET_ROUTE.exec(pathname);
    if (!match || match[1] !== currentPlanetId)
        return `/game/planets/${nextPlanetId}`;
