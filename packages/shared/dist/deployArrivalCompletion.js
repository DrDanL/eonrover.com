...
exports.completeOwnedPlanetDeployArrival = completeOwnedPlanetDeployArrival;
...
function canonicalManifest(value) {
    try {
        return (0, deployMission_1.canonicalDeployShips)(value);
    }
    catch {
        return null;
    }
 ...
 * as game state.
 */
async function completeOwnedPlanetDeployArrival(database, missionId, currentTime = new Date()) {
    ...
    for (let attempt = 0; attempt < SERIALIZABLE_TRANSACTION_ATTEMPTS; attempt += 1) {
        ...
        `;
                ...
                if (!mission || mission.missionType !== 'DEPLOY' || mission.status !== 'OUTBOUND')
                    return 'noop';
                if (mission.deployOriginId !== origin.id
