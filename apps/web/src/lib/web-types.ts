...
export interface ShipyardReadOnlyResponse {
  ...
  defences: Array<{ id: 'flakTurret' | 'railBattery' | 'planetaryShield'; name: string; displayOrder: number; availability: 'ACTIVE' | 'COMING_LATER'; availabilityReason: string; owned: number; cost: ResourceAmounts; durationSeconds: number; statistics: { attack: number; shield: number; armour: number }; requirements: Array<{ id: string; requiredLevel: number; currentLevel: number; met: boolean; type: 'building' | 'research' }>; meetsRequirements: boolean; affordable: boolean; quantity: { min: number; max: number }; fieldState: { capacity: number; used: number }; energyRequired: number }>;
