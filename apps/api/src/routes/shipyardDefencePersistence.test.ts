...
describe('canonical Shipyard defence marker persistence', () => {
  it('keeps representative legacy rows null-marked and permits only canonical Flak or Rail markers', async () => {
    const migrations = await prisma.$queryRaw<Array<{ migration_name: string }>>`
      SELECT "migration_name" FROM "_prisma_migrations"
      WHERE "migration_name" IN ('20260914100000_add_canonical_shipyard_defence_marker', '20260915000000_allow_canonical_rail_battery')
      ORDER BY "migration_name" ASC
    `;
    expect(migrations).toEqual([{ migration_name: '20260914100000_add_canonical_shipyard_defence_marker' }, { migration_name: '20260915000000_allow_canonical_rail_battery' }]);
    ...
    await expect(prisma.shipyardQueueItem.create({ data: { planetId: planet.id, itemKey: 'planetaryShield', itemType: 'defence', canonicalDefenceKey: 'planetaryShield', quantity: 1, remaining: 0, costAlloy: 1, costHeliox: 0, costAether: 0, durationSeconds: 1, status: 'COMPLETE', completesAt: new Date() } })).rejects.toThrow();
  });
});
