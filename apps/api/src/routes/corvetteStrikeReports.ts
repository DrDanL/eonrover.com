...
function target(value: unknown): SafeSnapshot['target'] | null {
  if (!isRecord(value) || !strictKeys(value, ['coordinates', 'planet']) || !isRecord(value.coordinates) || !isRecord(value.planet)
    || !strictKeys(value.coordinates, ['galaxy', 'slot', 'system']) || !strictKeys(value.planet, ['name', 'type'])) return null;
...
function forceGroups(value: unknown): { attacker: UnitMap; defender: UnitMap } | null {
  if (!isRecord(value) || !strictKeys(value, ['attacker', 'defender'])) return null;
  const attacker = units(value.attacker);
  const defender = units(value.defender);
  if (!attacker || !defender || Object.keys(attacker).some((key) => key !== 'corvette')) return null;
...
function safeSnapshot(value: unknown): SafeSnapshot | null {
  ...
  for (let index = 0; index < value.rounds.length; index += 1) {
    ...
    if (!isRecord(round) || !strictKeys(round, ['attackerLosses', 'defenderLosses', 'round']) || safeInteger(round.round) !== index + 1) return null;
...
function splitDefender(unitsValue: UnitMap): { ships: UnitMap; defences: UnitMap } {
  const ships: UnitMap = {}; const defences: UnitMap = {};
  for (const key of Object.keys(unitsValue).sort()) {
    if (key in SHIPS) ships[key] = unitsValue[key];
    if (key in DEFENCES) defences[key] = unitsValue[key];
  }
...
function attackerCounts(snapshot: SafeSnapshot) {
  return {
    startingCorvettes: snapshot.starting.attacker.corvette ?? 0,
    lostCorvettes: snapshot.losses.attacker.corvette ?? 0,
    survivingCorvettes: snapshot.survivors.attacker.corvette ?? 0,
  };
}
...
function presentDetail(row: CanonicalReportRow, snapshot: SafeSnapshot) {
  ...
  return {
    ...
    rounds: snapshot.rounds.map((round) => ({
      round: round.round,
      attackerLostCorvettes: round.attackerLosses.corvette ?? 0,
...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const [totalReports, reports] = await Promise.all([
    prisma.corvetteStrikeReport.count({ where }),
    prisma.corvetteStrikeReport.findMany({
      where,
...
router.get('/:reportId', asyncHandler(async (req, res) => {
  ...
  const report = await prisma.corvetteStrikeReport.findFirst({
